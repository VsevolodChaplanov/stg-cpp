...
namespace stg::generators {
     ...
     */
        ...
        SpectralGenerator(std::shared_ptr<AmplitudeGenerator> amplitude_generator,
            ...
            : config_{std::move(config)}, lower_triangular_{config_.reynolds_matrix_.cholesky()}, amplitude_generator_{std::move(amplitude_generator)}, wave_vector_generator_{std::move(wave_vector_generator)}, frequencies_generator_{std::move(frequencies_generator)}, wave_vectors_{wave_vectors(
     ...
     */
        ...
        Vector<value_type> operator()(const Point<value_type>& real_space_point, value_type time) const override {
            ...
            for (const size_t n: ranges::views::iota(0ul, config_.fourier_nodes_number_)) {
                const size_t phase = dot_product(wave_vectors_[n], scaled_vertex) + frequencies_[n] * scaled_time;
    ...
    public:
        ...
        ~KolmogorovSpectra() override = default;
    };
    ...
    public:
        ...
        SpectralGeneratorV2(std::shared_ptr<AmplitudeGenerator> amplitude_generator,
                            ...
                            value_type length_scale, value_type time_scale,
                            std::size_t spectra_n, std::size_t fourier_n)
            : spectra_n_{spectra_n}, fourier_modes_n_{fourier_n}, lower_triangular_{reynolds_tensor.cholesky()}, length_scale_{length_scale}, time_scale_{time_scale}, amplitude_generator_{std::move(amplitude_generator)}, wave_vector_generator_{std::move(wave_vector_generator)}, frequencies_generator_{std::move(frequencies_generator)}, rand_param_generator_{std::move(rand_param_generator)} {}
     ...
     */
        struct SpectralModeFluctuationGenerator final {
            ...
            void initialize_frequencies(std::shared_ptr<FrequenciesGenerator> freq_generator) {
                ...
                std::generate(std::execution::par, frequencies_.begin(), frequencies_.end(),
                              [gen = std::forward<std::shared_ptr<FrequenciesGenerator>>(freq_generator)] { return gen->operator()(); });
            }
        ...
        std::shared_ptr<ISpectra<value_type>> spectra_ = std::make_shared<VonKarmanSpectra<value_type>>(0.1, 10, 0.1);
