...
namespace bg = boost::geometry;
  ...
  public:
    ...
    value_type get(size_t i, size_t j) { return values_[i * 3 + j]; }
    ...
    operator*(const bg::model::point<value_type, 3, bg::cs::cartesian>& vector) const {
      const value_type x = vector.template get<0>() * values_[0]
        + vector.template get<1>() * values_[1] + vector.template get<2>() * values_[2];
      const value_type y = vector.template get<0>() * values_[3]
        + vector.template get<1>() * values_[4] + vector.template get<2>() * values_[5];
      const value_type z = vector.template get<0>() * values_[6]
        + vector.template get<1>() * values_[7] + vector.template get<2>() * values_[8];
      return {x, y, z};
    }

    [[nodiscard("Returns new tensor")]]
    Tensor operator*(value_type coeff) const {
      Tensor result = *this;
    ...
    [[nodiscard("Returns new lower triangular tensor")]]
    constexpr Tensor cholesky() const {
