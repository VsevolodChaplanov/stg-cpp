...
namespace rv = ranges::views;
  ...
  public:
    ...
    std::size_t lin_index(std::size_t i, std::size_t j, std::size_t k) const noexcept {
      return i + j * nx_ + k * nx_ * ny_;
    }

    std::array<std::size_t, 3> tri_index(std::size_t ivert) const noexcept {
    ...
    constexpr size_t element_type(std::size_t ielem) const override {
      return 12;
    }
    ...
    std::vector<Point<T>> element_vertices(std::size_t ielem) const override {
      ...
      std::for_each(bound_indices.cbegin(), bound_indices.cend(), [&result, this](auto index) {
        auto&& bound_vertex = vertex(index);
    ...
    const std::vector<std::vector<std::size_t>>& elements_global_indices() const noexcept override { return bound_indices_; }

    const std::vector<std::size_t>& elements_types() const noexcept override { return element_types_; }

    size_t n_vertices() const noexcept override { return n_vert_; }

    size_t n_elements() const noexcept override { return n_elem_; }
  ...
  public:
    ...
    CubeRelationTable(T L, std::size_t N,
      ...
      , bound_indices_{std::move(bound_indices)}
      , element_types_{std::move(elements_types)}
    { }
    ...
    const std::vector<std::vector<std::size_t>>& elements_global_indices() const noexcept override {
      return bound_indices_;
    }

    const std::vector<size_t>& elements_types() const noexcept override {
      return element_types_;
    }

    constexpr size_t n_vertices() const noexcept override {
      return n_ * n_ * n_;
    }

    constexpr size_t n_elements() const noexcept override{
    ...
    std::array<std::size_t, 3> center_tri_index() const {
      const std::size_t center_n = static_cast<std::size_t>((n_ - 1) / 2);
