...
namespace rv = ranges::views;
  ...
  public:
    ...
    [[nodiscard("Heavy object construction")]]
    std::shared_ptr<CubePrizmRelationTable<value_type>> build_relation_table() const {
    ...
    [[nodiscard("Heavy object construction")]]
    std::shared_ptr<CubePrizmFEMesh<value_type>> build() const {
  ...
  public:
    ...
    [[nodiscard("Heavy object construction")]]
    std::shared_ptr<CubeRelationTable<value_type>> build_relation_table() const {
    ...
    [[nodiscard("Heavy object construction")]]
    std::shared_ptr<CubeFiniteElementsMesh<value_type>> build() const {
      ...
      for (const size_t iz : rv::iota(0ul, n_ - 1)) {
        for (const size_t iy : rv::iota(0ul, n_ - 1)) {
          for (const size_t ix : rv::iota(0ul, n_ - 1)) {
            ...
            bounds.push_back({
              base_index, base_index + 1, base_index + n_ + 1, base_index + n_,
...
/*      ranges::view::iota(0ul, nelem) | ranges::for_each([&fe_elements, &rtable, this](size_t index) {
        const auto& element_indices = rtable->element_vertices_indices(index);
        const auto vertices_2d = rtable->element_vertices(index);
        fe_elements[index] = fe_factory_(vertices_2d, element_indices, FiniteElementsFactory::VtkTypes::Voxel);
      });*/
