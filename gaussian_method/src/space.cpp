...
namespace stg::kriging {
    ...
    double Space::use_interpolation_polynom(const std::array<size_t, 8>& indices, const std::array<double, 8>& bases, const std::vector<double>& v) const {
        return v[indices[0]] * bases[0] + v[indices[1]] * bases[1] + v[indices[2]] * bases[2] + v[indices[3]] * bases[3] + v[indices[4]] * bases[4] + v[indices[5]] * bases[5] + v[indices[6]] * bases[6] + v[indices[7]] * bases[7];
    }
