...
namespace rv = ranges::views;
  ...
  public:
    ...
    SpectralMethodApplicationImpl(std::shared_ptr<const CubeFiniteElementsMesh<T>> mesh,
                                  ...
                                  std::size_t samples_amount = 100,
                                  std::size_t concurrency_hint = std::thread::hardware_concurrency() - 1)
      : fe_mesh_{std::move(mesh)}
      , generator_config_(std::move(config))
      , spectral_generator_{generator_config_}
      , velocity_samples_{100, fe_mesh_->n_vertices()}
      , thread_pool_{new net::thread_pool{std::max(1ul, concurrency_hint)}}
    { }
