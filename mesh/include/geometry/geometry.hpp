...
namespace stg {
  ...
  Vector<T> cross_product(const Vector<T>& first, const Vector<T>& second) {
    const T xx = first.template get<1>() * second.template get<2>()
                 - first.template get<2>() * second.template get<1>();
    const T yy = first.template get<2>() * second.template get<0>()
                 - first.template get<0>() * second.template get<2>();
    const T zz = first.template get<0>() * second.template get<1>()
