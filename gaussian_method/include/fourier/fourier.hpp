...
namespace rv = ranges::views;
  ...
  std::vector<T> fourier3(const std::shared_ptr<mesh::CubeRelationTable<T>>& space, const std::vector<T>& f) {
    ...
    fftw_complex* input = (fftw_complex*)fftw_malloc(sizeof(fftw_complex)*n_vert);
    ...
    fftw_complex* output = (fftw_complex*) fftw_malloc(sizeof(fftw_complex) * n_vert);

    fftw_plan plan = fftw_plan_dft_3d(n_l, n_l, n_l, input, output,
